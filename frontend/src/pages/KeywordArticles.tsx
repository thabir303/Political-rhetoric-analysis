...
export function KeywordArticles() {
  ...
  const loadArticles = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8000/api/v1/keywords/${encodeURIComponent(keyword!)}/articles`
      );
  ...
  const handleKeywordClick = (clickedKeyword: string) => {
    navigate(`/keywords/${encodeURIComponent(clickedKeyword)}`);
  };
        ...
        {articles.length === 0 ? (
          <div className="bg-white rounded-lg shadow-lg p-12 text-center">
            ...
            <p className="text-gray-600">
              No articles found with this keyword. Try a different keyword or check back later.
            </p>
          </div>
        ) : (
